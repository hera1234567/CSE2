...
{      
        ...
        uint    R4toR11[CONTEXT_SIZE];   //saves registers in stack
        void    (*PC)();                
        uint    SPSR;                    //program status
...
typedef struct msgobj {
        char            *pData;         //pointer to msg
        exception       Status;         //from sender or reciever
        struct l_obj    *pBlock;        //pointer to block task waiting in waitinglist
...
typedef struct {
        msg             *pHead;
        msg             *pTail;
        int             nDataSize;              //Size of msg
        int             nMaxMessages;           //max amout of msgs allowed
        int             nMessages;              //amout of msgs in mailbox
        int             nBlockedMsg;            //amount of msgs blocking a task
} mailbox;
...
typedef struct l_obj {
         TCB            *pTask;
         uint           nTCnt;                  //Ticks counter, sleeptime 
         msg            *pMessage;              //pointer to message, sender or reciever
